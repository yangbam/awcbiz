/* ==========================================================================
   IPA site — management.html admin logic.
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function openMemberForm(member) {
      ...
      if (member) {
        ...
        memberForm.elements.category.value = member.category;
    ...
    function renderMembers() {
      ...
      memberTableBody.innerHTML = members.map(function (m) {
        ...
        return (
          "<tr>" +
          "<td>" + esc(m.name) + "</td>" +
          "<td>" + esc(MEMBER_CATEGORY_LABEL[m.category] || m.category) + "</td>" +
          "<td>" + esc(m.rep) + "</td>" +
          "<td>" + esc(m.field) + "</td>" +
    ...
    if (memberForm) {
      memberForm.addEventListener("submit", function (e) {
        ...
        var data = {
          ...
          category: memberForm.elements.category.value,
    ...
    function renderActivities() {
      ...
      activityTableBody.innerHTML = items.map(function (a) {
        var thumbCell = a.thumbnail
          ? '<img src="' + a.thumbnail + '" alt="" style="width:56px;height:38px;object-fit:cover;border-radius:var(--radius);border:1px solid var(--outline-variant);">'
          : '<span style="color:var(--on-surface-variant);">-</span>';
        var badges = "";
    ...
    function renderApplications() {
      ...
      applicationTableBody.innerHTML = apps.map(function (a) {
        ...
        return (
          ...
          "<td>" + esc(MEMBER_CATEGORY_LABEL[a.interestField] || a.interestField) + "</td>" +
          "<td>" + fileCell + "</td>" +
          "<td>" + statusBadge + "</td>" +
          '<td class="admin-table__actions">' +
    ...
    if (exportBtn) {
      exportBtn.addEventListener("click", function () {
        var payload = window.DataStore.exportAll();
        var blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" });
        var url = URL.createObjectURL(blob);
        var a = document.createElement("a");
        var stamp = new Date().toISOString().slice(0, 10);
        a.href = url;
        a.download = "ipa-data-backup-" + stamp + ".json";
